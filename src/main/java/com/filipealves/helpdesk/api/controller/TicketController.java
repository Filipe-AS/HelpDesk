package com.filipealves.helpdesk.api.controller;
...
import com.filipealves.helpdesk.api.dto.Summary;
import com.filipealves.helpdesk.api.entity.ChangeStatus;
import com.filipealves.helpdesk.api.entity.Ticket;
import com.filipealves.helpdesk.api.entity.User;
import com.filipealves.helpdesk.api.enums.ProfileEnum;
import com.filipealves.helpdesk.api.enums.StatusEnum;
import com.filipealves.helpdesk.api.response.Response;
import com.filipealves.helpdesk.api.security.jwt.JwtTokenUtil;
import com.filipealves.helpdesk.api.service.TicketService;
...
public class TicketController {
	...
	public ResponseEntity<Response<Ticket>> create(HttpServletRequest request, @RequestBody Ticket ticket,
		...
		try {
			validationCreateTicket(ticket, result);
			if (result.hasErrors()) {
	...
	private void validationCreateTicket(Ticket ticket, BindingResult result) {
	...
	public ResponseEntity<Response<Ticket>> update(HttpServletRequest request, @RequestBody Ticket ticket,
		...
		try {
			validationUpdateTicket(ticket, result);
			if (result.hasErrors()) {
				result.getAllErrors().forEach(error -> response.getErrors().add(error.getDefaultMessage()));
				return ResponseEntity.badRequest().body(response);
			}
	...
	private void validationUpdateTicket(Ticket ticket, BindingResult result) {
	...
	@GetMapping(value ="{page}/{count}/{numer}/{title}/{status}/{priority}/{assigned}")
	@PreAuthorize("hasAnyRole('CUSTOMER,TECHNICIAN')")
	public ResponseEntity<Response<Page<Ticket>>> findByParams(HttpServletRequest request, 
											@PathVariable("page") int page,
											@PathVariable("count") int count,
											@PathVariable("numer") Integer number,
	...
	public ResponseEntity<Response<Ticket>> changeStatus(
		...
		Response<Ticket> response = new Response<Ticket>();
		try {
			validationChangeStatus(id,status,result);
			if (result.hasErrors()) {
				result.getAllErrors().forEach(error -> response.getErrors().add(error.getDefaultMessage()));
				return ResponseEntity.badRequest().body(response);
			}
	...
	private void validationChangeStatus(String id, String status, BindingResult result) {
